...
namespace utils
{
	...
	strict_inline simd_float map(simd_float value)
	{
		simd_float result;
	...
	static strict_inline simd_float map(simd_float one, simd_float two) {
		simd_float result;
		for (int i = 0; i < simd_float::kSize; ++i)
	...
	strict_inline simd_float sin(simd_float value) { return map<sinf>(value); }
	strict_inline simd_float cos(simd_float value) { return map<cosf>(value); }
	strict_inline simd_float tan(simd_float value) { return map<tanf>(value); }
	strict_inline simd_float atan2(simd_float one, simd_float two) { return map<atan2f>(one, two); }

	strict_inline simd_float sqrt(simd_float value)
	{
	#if COMPLEX_SSE3
	...
	strict_inline simd_float interpolate(simd_float &from, simd_float &to, float t)
	{ return simd_float::mulAdd(from, to - from, t); }

	strict_inline simd_float interpolate(simd_float &from, simd_float &to, simd_float &t)
	{ return simd_float::mulAdd(from, to - from, t); }

	strict_inline simd_float interpolate(float from, float to, simd_float &t)
	{ return simd_float::mulAdd(from, to - from, t); }

	// t are clamped to the range of low and high
	strict_inline simd_float interpolateClamp(simd_float &from, simd_float &to,
	...
	strict_inline Framework::matrix getLinearInterpolationMatrix(simd_float t)
	{ return Framework::matrix({ 0.0f, simd_float(1.0f) - t, t, 0.0f }); }

	strict_inline Framework::matrix getCatmullInterpolationMatrix(simd_float t)
	{
	...
	strict_inline simd_float toSimdFloatFromUnaligned(const float *unaligned)
	{
	#if COMPLEX_SSE3
	...
	strict_inline Framework::matrix getValueMatrix(const float *buffer, simd_int indices)
	{
	...
	template<u32 size> requires requires () { size <= kSimdRatio; }
	strict_inline Framework::matrix getValueMatrix(const float *const *buffers, simd_int indices)
	{
		std::array<simd_float, size> values;
		for (u32 i = 0; i < values.size(); i++)
	...
	strict_inline simd_float clamp(simd_float value, float min, float max)
	{ return simd_float::max(simd_float::min(value, max), min); }

	strict_inline simd_float clamp(simd_float value, simd_float min, simd_float max)
	{ return simd_float::max(simd_float::min(value, max), min); }

	strict_inline simd_int clamp(simd_int value, simd_int min, simd_int max)
	{ return simd_int::max(simd_int::min(value, max), min); }

	strict_inline bool completelyEqual(simd_float left, simd_float right)
	{ return simd_float::notEqual(left, right).sum() == 0; }

	strict_inline simd_float maskLoad(simd_float oneValue, simd_float zeroValue, simd_mask mask)
	{ return (oneValue & mask) + (zeroValue & ~mask); }

	strict_inline simd_int maskLoad(simd_int oneValue, simd_int zeroValue, simd_mask mask)
	{ return (oneValue & mask) | (zeroValue & ~mask); }
	...
	strict_inline void copyBuffer(simd_float *dest, const simd_float *source, int size)
	{
		for (int i = 0; i < size; ++i)
			dest[i] = source[i];
	}

	strict_inline void addBuffers(simd_float *dest, const simd_float *b1, const simd_float *b2, int size)
	{
		for (int i = 0; i < size; ++i)
			dest[i] = b1[i] + b2[i];
	}

	strict_inline simd_float toFloat(simd_int integers)
	{
	#if COMPLEX_SSE3
	...
	strict_inline simd_int toInt(simd_float floats)
	{
	#if COMPLEX_SSE3
	...
	strict_inline simd_float reinterpretToFloat(simd_int value)
	{
	#if COMPLEX_SSE3
	...
	strict_inline simd_int reinterpretToInt(simd_float value)
	{
	#if COMPLEX_SSE3
	...
	strict_inline simd_float truncate(simd_float value)
	{ return toFloat(toInt(value)); }

	strict_inline simd_float floor(simd_float value)
	{
		simd_float truncated = truncate(value);
		return truncated + (simd_float(-1.0f) & simd_float::greaterThan(truncated, value));
	}

	strict_inline simd_float ceil(simd_float value)
	{
		simd_float truncated = truncate(value);
		return truncated + (simd_float(1.0f) & simd_float::lessThan(truncated, value));
	}

	strict_inline simd_int floorToInt(simd_float value)
	{ return toInt(floor(value)); }

	strict_inline simd_int ceilToInt(simd_float value)
	{ return toInt(ceil(value)); }

	strict_inline simd_int roundToInt(simd_float value)
	{ return floorToInt(value + 0.5f); }

	strict_inline simd_float round(simd_float value)
	{ return floor(value + 0.5f); }

	strict_inline simd_float mod(simd_float value)
	{ return value - floor(value); }

	template<size_t shift>
	strict_inline simd_int shiftRight(simd_int values)
	{
	#if COMPLEX_SSE3
	...
	template<size_t shift>
	strict_inline simd_int shiftLeft(simd_int values)
	{
	#if COMPLEX_SSE3
	...
	template<size_t shift>
	strict_inline simd_float shiftRight(simd_float value)
	{ return reinterpretToFloat(shiftRight<shift>(reinterpretToInt(value))); }

	template<size_t shift>
	strict_inline simd_float shiftLeft(simd_float value)
	{ return reinterpretToFloat(shiftRight<shift>(reinterpretToInt(value))); }

	strict_inline simd_float pow2ToFloat(simd_int value)
	...
	strict_inline simd_float exp2(simd_float exponent)
	{
	...
	strict_inline simd_float log2(simd_float value)
	{
	...
	strict_inline simd_float exp(simd_float exponent)
	{ return exp2(exponent * kExpConversionMult); }

	strict_inline simd_float log(simd_float value)
	{ return log2(value) * kLogConversionMult; }

	strict_inline simd_float pow(simd_float base, simd_float exponent)
	{ return exp2(log2(base) * exponent); }

	strict_inline simd_float midiOffsetToRatio(simd_float note_offset)
	{ return exp2(note_offset * (1.0f / kNotesPerOctave)); }

	strict_inline simd_float midiNoteToFrequency(simd_float note)
	{ return midiOffsetToRatio(note) * kMidi0Frequency; }

	// fast approximation of the original equation
	strict_inline simd_float magnitudeToDb(simd_float magnitude)
	{ return log2(magnitude) * kDbGainConversionMult; }

	// fast approximation of the original equation
	strict_inline simd_float dbToMagnitude(simd_float decibels)
	...
	strict_inline simd_mask unsignInt1(simd_int &value)
	{
		simd_mask mask = simd_mask::equal(value & simd_mask(kSignMask), simd_mask(kSignMask));
		value = value ^ (mask & simd_int(kSignMask));
		return mask;
	}

	// conditionally unsigns floats if they are < 0 and returns mask
	strict_inline simd_mask unsignFloat(simd_float &value)
	{
