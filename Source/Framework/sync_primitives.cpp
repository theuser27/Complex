...
extern "C"
{
  using NtDelayExecution_t = NTSTATUS __stdcall(unsigned char Alertable, PLARGE_INTEGER Interval);
}
...
namespace utils
{
  ...
  i32 lockAtomic(LockBlame<i32> &lock, bool isExclusive, WaitMechanism mechanism, const clg::small_function<void()> &lambda) noexcept
  {
    ...
    if (isExclusive)
    {
      ...
      while (!lock.lock.compare_exchange_weak(state, desired, std::memory_order_acq_rel, std::memory_order_acquire))
      {
        ...
        if (state <= expected)
        {
          COMPLEX_ASSERT(lock.lastLockId.load(std::memory_order_relaxed) != std::this_thread::get_id(),
      ...
      lock.lastLockId.store(std::this_thread::get_id(), std::memory_order_relaxed) ;
    }
    else
    {
      state = lock.lock.load(std::memory_order_relaxed);
  ...
  void unlockAtomic(LockBlame<i32> &atomic, bool wasExclusive, WaitMechanism mechanism) noexcept
  {
    if (wasExclusive)
    {
      atomic.versionFlag.fetch_add(1);
