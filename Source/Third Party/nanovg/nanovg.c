...
#define FONS_MALLOC NVG_MALLOC
#define FONS_REALLOC NVG_REALLOC
#define FONS_FREE NVG_FREE
...
NVGcolor nvgFromHSB(NVGcolor color)
{
	color.b = nvg__clampf(0.0f, 1.0f, color.b);
	...
	color.g = nvg__minf(1.0f, color.g);
	color.r = ((color.r - floorf(color.r)) * 360.0f) / 60.0f;
	float f = color.r - floorf(color.r);
	float x = color.b * (1.0f - color.g);

	float h = color.r;
	float s = color.g;
	float v = color.b;

	if (h < 1.0f) return (NVGcolor){ .r = v, .g = v * (1.0f - (s * (1.0f - f))), .b = x, .a = color.a };
	if (h < 2.0f) return (NVGcolor){ .r = v * (1.0f - s * f), .g = v, .b = x, .a = color.a };
	if (h < 3.0f) return (NVGcolor){ .r = x, .g = v, .b = v * (1.0f - (s * (1.0f - f))), .a = color.a };
	if (h < 4.0f) return (NVGcolor){ .r = x, .g = v * (1.0f - s * f), .b = v, .a = color.a };
	if (h < 5.0f) return (NVGcolor){ .r = v * (1.0f - (s * (1.0f - f))), .g = x, .b = v, .a = color.a };
	return               (NVGcolor){ .r = v, .g = x, .b = v * (1.0f - s * f), .a = color.a };
}

static float nvg__getHue(float r, float g, float b, float hi, float lo)
{
	float hue = 0.0f;

	if (hi > 0 && hi != lo)
	{
		float invDiff = 1.0f / (float)(hi - lo);

		float red   = (float)(hi - r) * invDiff;
		float green = (float)(hi - g) * invDiff;
		float blue  = (float)(hi - b) * invDiff;
...
NVGcolor nvgHSB(NVGcolor color)
{
	float hi = nvg__maxf(nvg__maxf(color.r, color.g), color.b);
	float lo = nvg__minf(nvg__minf(color.r, color.g), color.b);
	...
	if (hi > 0.0f)
	{
		...
		if (saturation > 0.0f)
			hue = nvg__getHue(color.r, color.g, color.b, hi, lo);
