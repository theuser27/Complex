...
namespace Interface
{
	EffectsStateSection::EffectsStateSection(Generation::EffectsState &state) :
		ProcessorSection{ &state }, state_{ state }
	{
	...
	void EffectsStateSection::releaseComponent(EffectModuleSection *movedModule, const MouseEvent &)
	{
		...
		auto &processorTree = *getProcessor()->processorTree;
		...
		if (movedModuleCopy_ != nullptr)
		{
			...
			update = new Framework::AddProcessorUpdate{ processorTree,
				lanes_[newIndices.first]->processorId.value(), newIndices.second, *processor };
		}
		else
		{
			...
			update = new Framework::MoveProcessorUpdate(processorTree, lanes_[newIndices.first]->processorId.value(),
				newIndices.second, lanes_[dragStartIndices_.first]->processorId.value(), dragStartIndices_.second);
		}
