...
typedef HRESULT(WINAPI *PFN_GetProcessDpiAwareness)(HANDLE, PROCESS_DPI_AWARENESS *);
...
handleMessage(PuglView* view, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_SHOWWINDOW:
    ...
    break;
  case WM_DPICHANGED:
    event.type = PUGL_DPI_CHANGE;
    event.dpiChange.newScaleFactor = (double)HIWORD(wParam) / USER_DEFAULT_SCREEN_DPI;
    break;
  ...
  case WM_PAINT:
    ...
    ValidateRect(view->impl->hwnd, NULL);
    break;
  case WM_ERASEBKGND:
    return true;
  case WM_MOUSEMOVE:
  ...
  case WM_LBUTTONDOWN:
  case WM_LBUTTONUP:
    initMouseEvent(&event, view, 0, message == WM_LBUTTONDOWN, lParam);
    break;
  case WM_RBUTTONDOWN:
  case WM_RBUTTONUP:
    initMouseEvent(&event, view, 1, message == WM_RBUTTONDOWN, lParam);
    break;
  case WM_MBUTTONDOWN:
  case WM_MBUTTONUP:
    initMouseEvent(&event, view, 2, message == WM_MBUTTONDOWN, lParam);
    break;
  case WM_XBUTTONDOWN:
  case WM_XBUTTONUP:
    if (GET_XBUTTON_WPARAM(wParam) == XBUTTON1) {
      initMouseEvent(&event, view, 3, message == WM_XBUTTONDOWN, lParam);
    } else {
      initMouseEvent(&event, view, 4, message == WM_XBUTTONDOWN, lParam);
    }
    break;
  case WM_MOUSEWHEEL:
  ...
  case PUGL_LOCAL_CLIENT_MSG:
    ...
    memcpy(&event.client.data2, &lParam, sizeof(lParam));
    break;
  case WM_QUIT:
