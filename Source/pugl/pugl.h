...
typedef enum {
  ...
  PUGL_DPI_CHANGE,     ///< DPI changed, a #PuglDPIChangeEvent
...
typedef enum {
  ...
  /// View is beginning/ending to be moved/resized
  PUGL_VIEW_STYLE_BEGIN_END_GESTURE = 1U << 9U,

  /// View is ready for input or otherwise demanding attention
  PUGL_VIEW_STYLE_DEMANDING = 1U << 10U,
} PuglViewStyleFlag;
...
typedef struct
{
  PuglEventType  type;           ///< #PUGL_DPI_CHANGE
  PuglEventFlags flags;          ///< Bitwise OR of #PuglEventFlag values
...
// TODO: change PUGL_MOD_CTRL to mean command on macos
typedef enum {
...
typedef struct {
  ...
  char           data2[32]; ///< Client-specific data
} PuglClientEvent;
...
typedef union {
  ...
  PuglDPIChangeEvent dpiChange; ///< #PUGL_DPI_CHANGE
