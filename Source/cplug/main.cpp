...
uint32_t cplug_getParameterCount(void *ptr) { return CPLUG_NUM_PARAMS; }
...
class Timer
{
  static constexpr double kMicro = 1000000.0;

  /// static state
  //
  int32_t sleepUs_ = 0;
  uint64_t systemFrequency_ = []()
  {
    LARGE_INTEGER largeInt;
    QueryPerformanceFrequency(&largeInt);
    return (int64_t)largeInt.QuadPart;
  }();
  uint64_t correctionPeriod_ = systemFrequency_; // corrects every second by default

  /// runtime state
  //
  int32_t correctionUs_ = 500; // start at some reasonable correction
  uint32_t counter_ = 0;
  uint64_t correctionStart_ = 0;

public:
  void setPeriod(double microseconds)
  {
    sleepUs_         = (int32_t)microseconds;
    correctionStart_ = 0;
    correctionUs_ = 500;
    counter_ = 0;
  }
  ...
  {
    correctionPeriod_ = (int64_t)((double)systemFrequency_ * seconds);
    correctionStart_ = 0;
    correctionUs_ = 500;
    counter_ = 0;
  }
  ...
  void correctSleep()
  {
    uint64_t timestamp;
    {
      ...
      timestamp = (uint64_t)largeInt.QuadPart;
    }
    ...
    double observedPeriod = (kMicro / (double)(counter_ * systemFrequency_)) * elapsedTime;
    correctionUs_ += (int32_t)observedPeriod - sleepUs_;
...
void spinAtomic(std::atomic<bool> &atomic, bool expected) noexcept
{
  bool state = expected;
  while (!atomic.compare_exchange_weak(state, !expected, std::memory_order_acq_rel, std::memory_order_relaxed))
  {
    ...
    while (atomic.load(std::memory_order_relaxed) != expected) { _mm_pause(); }
...
{
public:
  ScopedLock(std::atomic<bool>& atomic, bool expected = false) noexcept : 
    atomic_{&atomic}, expected_{expected} { spinAtomic(atomic, expected); }
  ...
  ~ScopedLock() noexcept { atomic_->store(expected_, std::memory_order_release); }
...
struct GlThread {
  enum Flags { Wait, Shutdown, Setup, Teardown, Configure };

  PuglGUI *          app        = nullptr;
  PuglArea           area       = {};
  double             fps        = 60.0;
  std::atomic<Flags> flag       = Wait;
  std::atomic<bool>  isResizing = false;
  std::atomic<bool>  dataGuard  = false;
  std::thread        thread{};

  bool isRunning() { return thread.get_id() != std::thread::id{}; }

  void shutdownGlThread()
  {
    flag.store(GlThread::Shutdown, std::memory_order_release);
    flag.notify_one();
    thread.join();
  }
  ...
  void runThread()
  {
    ...
    auto resetFlag = [&]() {
      flag.store(Wait, std::memory_order_release);
      flag.notify_all();
    };

    auto deinitialise = [&]() {
      teardownGl(app);
      puglLeaveContext(app->view);
      isInitialised = false;
    };

    // de/initialisation loop
    while (true) {
      ...
      // initialise
      {
        ...
        while (true) {
          flag_ = flag.load(std::memory_order_acquire);
          if (flag_ == Setup || flag_ == Shutdown)
            break;
          else {
            // defensive measure in case we get other flags
            // we don't care about them at this point
            resetFlag();
          }

          // Relaxed because we'll always do the load with acquire
          flag.wait(flag_, std::memory_order_relaxed);
        }
        ...
        {
          ScopedLock g{dataGuard};

          assert(app != nullptr && "App not set");

          PuglStatus status = puglEnterContext(app->view);
      ...
      // wait for configuration
      {
        while (true) {
          flag_ = flag.load(std::memory_order_acquire);
          if (flag_ == Configure || flag_ == Shutdown || flag_ == Teardown)
            break;
        ...
        if (flag_ == Shutdown || flag_ == Teardown) {
          deinitialise();

          if (flag_ == Shutdown)
            return;
          else if (flag_ == Teardown)
            continue;
        }

        {
          ScopedLock g{dataGuard};
          onConfigure(app->view, area.width, area.height);
      ...
      while (true) {

        {
          ...
          onExpose(app->view);
          puglSwapBuffers(app->view);
          if (isResizing.load(std::memory_order_acquire))
            glFinish();
        }
...
{
  PuglGUI* app = (PuglGUI *)puglGetHandle(view);

  switch (event->type) {
  case PUGL_REALIZE:
    puglLeaveContext(view);
    {
      ScopedLock g{app->glThread->dataGuard};
      app->glThread->app = app;
    }
    if (!app->glThread->isRunning())
      break;
    
    app->glThread->flag.store(GlThread::Setup, std::memory_order_release);
    app->glThread->flag.notify_one();
    app->glThread->flag.wait(GlThread::Setup, std::memory_order_relaxed);
  ...
  case PUGL_UNREALIZE:
    ...
    app->glThread->flag.store(GlThread::Teardown, std::memory_order_release);
    app->glThread->flag.notify_one();
    app->glThread->flag.wait(GlThread::Teardown, std::memory_order_relaxed);
    app->glThread->app = nullptr;
    
    break;
  case PUGL_CONFIGURE:
    if ((view->lastConfigure.style & PUGL_VIEW_STYLE_RESIZING) == (event->configure.style & PUGL_VIEW_STYLE_RESIZING))
    {
      app->glThread->isResizing.store(
        (event->configure.style & PUGL_VIEW_STYLE_RESIZING) != 0,
        std::memory_order_relaxed);
    }

    if (view->lastConfigure.width == event->configure.width &&
        view->lastConfigure.height == event->configure.height)
      break;
    view->lastConfigure = event->configure;

    {
      ScopedLock g{app->glThread->dataGuard};
      app->glThread->area.width = event->configure.width;
      app->glThread->area.height = event->configure.height;
    }

    if (!app->glThread->isRunning())
      break;

    app->glThread->flag.store(GlThread::Configure, std::memory_order_release);
    app->glThread->flag.notify_one();
    app->glThread->flag.wait(GlThread::Configure, std::memory_order_relaxed);
...
{
  MyPlugin *plugin = (MyPlugin *)userPlugin;
  PuglGUI *app = (PuglGUI *)malloc(sizeof(PuglGUI));
  memset(app, 0, sizeof(*app));

  app->plugin = plugin;
  plugin->gui = app;
  app->glThread = new GlThread();

  // Create world, view, and rect data
  app->world = puglNewWorld(PUGL_MODULE, 0);
  ...
  puglSetSizeHint(app->view, PUGL_DEFAULT_SIZE, GUI_DEFAULT_WIDTH, GUI_DEFAULT_HEIGHT);
  ...
  puglSetViewHint(app->view, PUGL_CONTEXT_DEBUG, PUGL_TRUE);
  puglSetViewHint(app->view, PUGL_RESIZABLE, PUGL_TRUE);
  puglSetViewHint(app->view, PUGL_SAMPLES, 1);
  puglSetViewHint(app->view, PUGL_DOUBLE_BUFFER, PUGL_TRUE);
  puglSetViewHint(app->view, PUGL_SWAP_INTERVAL, 1);
...
{
  PuglGUI *gui = (PuglGUI *)userGUI;
  gui->plugin->gui = NULL;

  puglUnrealize(gui->view);
  gui->glThread->shutdownGlThread();
  delete gui->glThread;
...
void cplug_setParent(void *userGUI, void *newParent)
{
  ...
  if (puglGetParent(gui->view))
  {
    puglSetParent(gui->view, (PuglNativeView)NULL);
    puglUnrealize(gui->view);
    gui->initialised = false;
  }
  if (newParent)
  {
    puglSetParent(gui->view, (PuglNativeView)newParent);
    gui->glThread->restartThread();
    PuglStatus status = puglRealize(gui->view);
    assert(status == PUGL_SUCCESS);
...
void cplug_getSize(void *userGUI, uint32_t *width, uint32_t *height)
{
  ...
  PuglArea size;
  if (gui->initialised)
    size = puglGetSizeHint(gui->view, PUGL_CURRENT_SIZE);
  else
    size = puglGetSizeHint(gui->view, PUGL_DEFAULT_SIZE);
...
bool cplug_setSize(void *userGUI, uint32_t width, uint32_t height)
{
  ...
  puglSetSizeHint(gui->view, PUGL_CURRENT_SIZE, width, height);
  return true;
}
