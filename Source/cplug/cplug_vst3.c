...
const char* _cplug_tuid2str(const Steinberg_TUID iid)
{
    ...
    static char buf[46];
    stbsp_snprintf(buf, sizeof(buf), "{0x%08X,0x%08X,0x%08X,0x%08X}", idu32[0], idu32[1], idu32[2], idu32[3]);
    return buf;
}
    ...
    struct Steinberg_Vst_NoteExpressionTypeInfo* info)
{
    ...
    if (busIndex == 0 && channel == 0 && noteExpressionIndex == 0)
    {
        ...
        // set some strings
        memcpy(info->title, L"Tuning", sizeof(L"Tuning"));
        memcpy(info->shortTitle, L"Tun", sizeof(L"Tun"));
        memcpy(info->units, L"Half Tone", sizeof(L"Half Tone"));
